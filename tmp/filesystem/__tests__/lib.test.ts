...
describe('Lib Functions', () => {
  beforeEach(() => {
    ...
    const allowedDirs = [
      ...
      { hostPath: '/allowed', mcpPath: '/allowed', readonly: false },
      { hostPath: '/readonly', mcpPath: '/readonly', readonly: true },
    ];
  ...
  describe('Security & Validation Functions', () => {
    describe('resolveMcpPath', () => {
      ...
      it('resolves with custom mcp path mapping', async () => {
        // /allowed maps to /host/data
        const result = await resolveMcpPath('/allowed/test.txt');
        expect(result).toBe('/allowed/test.txt');
      });
    ...
    describe('isMcpPathReadonly', () => {
      it('returns true for readonly directories', () => {
        expect(isMcpPathReadonly('/readonly/file.txt')).toBe(true);
        expect(isMcpPathReadonly('/readonly')).toBe(true);
        expect(isMcpPathReadonly('/readonly/subdir/file.txt')).toBe(true);
      });
      ...
      it('returns false for readwrite directories', () => {
        ...
        expect(isMcpPathReadonly('/allowed/file.txt')).toBe(false);
      });
    ...
    describe('validatePath', () => {
      ...
      it('rejects write to readonly directory', async () => {
        await expect(validatePath('/readonly/file.txt', true))
          .rejects.toThrow('Access denied - directory is read-only');
      });

      it('allows read from readonly directory', async () => {
        const result = await validatePath('/readonly/file.txt', false);
        expect(result).toBe('/readonly/file.txt');
      });
    });
  });
  ...
  describe('Search & Filtering Functions', () => {
    describe('searchFilesWithValidation', () => {
      ...
      it('excludes files matching exclude patterns', async () => {
        ...
        expect(result).toEqual(['/allowed/dir/test.txt']);
      });
